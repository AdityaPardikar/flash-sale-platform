...
// Request type augmentation is in types/express.d.ts
 ...
 */
function logResponse(req: Request, res: Response, _body?: unknown): void {
  const duration = Date.now() - (req.startTime || Date.now());
 ...
 */
export const skipLoggingPaths = ['/api/health/live', '/api/health/ready', '/favicon.ico'];
